...
- [~] **Phase G2 — Forward skeleton.** *(runs end-to-end 2026-07-01; coherence gated on G3 PLE)*
  - Dedicated gemma4 path in `src/cli/main.cpp` (guarded by `arch=="gemma4"`; Llama/Qwen loop untouched): √hidden embedding scale, `(1+w)` RMSNorm, 4 sandwich norms (attn_norm / post_attention_norm / ffn_norm / post_ffw_norm), per-head QK-norm, **per-layer head_dim** (256 local / 512 global, derived from `attn_q` shape), GQA broadcast (num_kv_heads=1→8), **KV sharing** (layers 15–34 reuse the last same-type layer's KV via an own per-layer store, not `model.kv_cache()`), NeoX RoPE with per-type freq base, GeGLU (tanh-GELU) FFN at per-layer width, final logit soft-cap (30). Norms/RoPE/GeGLU on host; matmuls on NPU. **Deferred:** PLE (`inp_gate`/`proj`/`per_layer_*`), `post_norm`, `layer_output_scale`, SWA masking.
  - **Q4_0 support fixed along the way** (gemma is entirely Q4_0; no prior model used it, so these were latent): `type_sizes[Q4_0]` 16→**18** (root cause of a decode-fallback → OOB segfault); `decode_q4_0_for_npu` rewritten to per-row int8 + per-row scale (was per-block); `dequant_tensor_row` gains a Q4_0 case (embedding had been decoding to zeros → all-`<pad>`); Q4_0 added to matmul `kq_path`, `attach_kquant_scales`, `compute_logits` NPU path, and a `w4_` weight-cache version prefix.
  - **Milestone:** runs end-to-end through all 35 layers, emits varied (non-degenerate) tokens; **not yet coherent** — expected, Gemma 4 needs PLE (cf. llama.cpp #22243). Llama/Qwen2 verified unregressed.
- [x] **Phase G3 — PLE pathway → COHERENT.** *(done 2026-07-01)*
  - PLE built per token: token-identity (`per_layer_token_embd[tok]` × √ple_dim) + context-aware (RMSNorm(`per_layer_model_proj`(embed) / √hidden)), combined ×(1/√2); injected per layer via `post_norm(proj(gelu(inp_gate(h)) · pli_L))`, residual `h += inj`.
  - **Ground truth:** read the exact `llama.cpp/src/models/gemma4.cpp` forward graph. Three bugs fixed against it: **(1)** V is plain RMS-normed (`ggml_rms_norm(Vcur, eps)`, no weight, no RoPE) before attention — was missing; **(2)** `layer_output_scale` multiplies the **full** hidden (`cur *= out_scale`), magnitude renormalized by the next RMSNorm — not a delta; **(3)** attention scale is **1.0** (`f_attention_scale`), so Q is pre-scaled by √head_dim to cancel the host flash_attn's 1/√hd. Confirmed already-correct: raw-weight RMSNorm, QK-norm→RoPE order, GeGLU FFN, sandwich-norm residual structure, √hidden embed scale, softcap 30. Q4_0 matmul validated (rel-L2 1.9%, `GGNPU_GEMMA_DEBUG=1`).
...
- [x] **Phase G5 — Finishing + validation.** *(done 2026-07-01)*
  ...
  - **Deferred to Phase 6 (perf, not correctness):** originally the gemma4 path ran RMSNorm / QK-norm / RoPE / GeGLU / attention on the **host** (only matmuls on the NPU); decode was ~1 tok/s.

### 7.3.1 Gemma4 Phase 6 — NPU perf offload (branch `gemma4-perf`)

- [x] **Phase P1 — RMSNorm/QK-norm NPU offload.** *(done 2026-07-01)* All 10 gemma4 RMSNorm call sites (sandwich norms, per-head QK-norm, K/V-norm, PLE context-norm) now dispatch through `backend->rms_norm()`. hidden=1536 reuses the existing `rmsnorm_2048` xclbin via next_pow2 padding; new shaped `rmsnorm_256`/`rmsnorm_512` xclbins cover QK-norm/PLE sizes. Fixed two latent bugs surfaced by real gemma4 data: the NPU row-duplication logic was keyed off N_pad size instead of kernel type, and the compiled RMSNorm kernel bakes `eps=1e-5` at compile time (invisible for Llama/Qwen2's ~1e-5 eps, but a large relative error for gemma4's `eps=1e-6` on near-zero QK-norm/PLE-norm activations) — replaced the N/N_pad-only output correction with an exact correction computed from the real input.
- [x] **Phase P2 — RoPE NPU offload (NeoX split-half).** *(done 2026-07-01)* Added a `neox_split_half` flag to `RopeParams`/`RopeBatchedParams` (default false; Llama/Qwen2 unaffected) so the NPU RoPE kernel's pure-elementwise-add math can be reused with Gemma4's `data[i]`/`data[i+half]` pair layout instead of Llama's adjacent-pair `data[2i]`/`data[2i+1]`. New shaped `rope_256`/`rope_512` xclbins (head_dim 256/512); also fixed a missing prebuilt-shaped-xclbin lookup in `ensure_rope_kernel()`.
- [x] **Phase P3 — GeGLU/GELU NPU offload (new kernel).** *(done 2026-07-01)* Added the project's first NPU GELU kernel (tanh-approx, reusing SiLU's math-agnostic MLIR transform) plus `Backend::gelu()`/`GeluParams`, wired into gemma4's two GeGLU sites. Found and fixed two bugs via a new validation gate: `ensure_gelu_kernel()`'s shaped-file lookup ignored the requested size (so N=12288 wrongly reused the fixed-8192 kernel, leaving the tail undefined), and the validator initially read the in-place-overwritten input as if it were pre-computation (fixed by snapshotting before dispatch).
- **Result:** correctness preserved throughout (golden `test_e2e_gemma` — France → Paris — passes after every phase); decode/prefill measured at ~0.61 forward-passes/sec post-P3 (13 passes — 6 prompt + 7 generated — in ~21.5s wall including model load; note: an earlier version of this note miscalculated this as "~1.6 forward-passes/sec" — that inverted seconds/pass and passes/sec), roughly in line with the ~1 tok/s pre-offload baseline (P1-P3 mainly fixed correctness/enabled NPU dispatch; see below for the actual perf win).
- **Profiled with new `-v` timing instrumentation (2026-07-01):** **matmul dispatch dominates (77.2% matmul + 12.8% logits ≈ 90% combined); flash_attn (host f32) is only 0.19% — not the bottleneck**, contradicting the guess above. Root cause matches the pre-existing Llama finding (decode is device-bound on matmul launch overhead, ~26µs/tile) — gemma4 has 9 small matmul launches/layer × 35 layers ≈ 315 launches/token, more than Llama, so it's hit harder.
- **Phase P5 — generalize deep-K matmul to K=1536 (done 2026-07-01, commit 393f717).** The existing deep-K decode-matmul optimization (one in-kernel-K-reduction launch replacing up to K/256 plain-tile launches) required `K % 2048 == 0`; 7 of gemma4's 9 per-layer matmuls have K=1536 (hidden_size), missing it entirely and falling back to 6 plain launches each. Qwen2 (also hidden=1536) was equally affected. A K=1536-exact kernel isn't buildable (Triton's `tl.arange` requires a power-of-2 `BLOCK_SIZE_K`), so instead relaxed the dispatch condition to `K > kMatmulTile`: the existing K-tiling loop already zero-pads a partial final tile, so K=1536 gets one padded launch through the existing K=2048 kernel (exact — zero padding contributes nothing to the dot product) — no new xclbin needed. **Result: ~26% faster wall-clock** (21.5s → 17.1s for the same 13-pass golden run; 0.61 → 0.76 passes/sec). Validated against Llama (`test_e2e_logits`), gemma4 (`test_e2e_gemma`), and Qwen2.5-Coder-1.5B (manual, unregressed and also benefits).
