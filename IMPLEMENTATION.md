...
- **Result:** correctness preserved throughout (golden `test_e2e_gemma` — France → Paris — passes after every phase); decode/prefill measured at ~1.6 forward-passes/sec post-P3 (13 passes — 6 prompt + 7 generated — in ~21.5s wall including model load), up from the ~1 tok/s pre-offload baseline.
- **Profiled with new `-v` timing instrumentation (2026-07-01):** **matmul dispatch dominates (77.2% matmul + 12.8% logits ≈ 90% combined); flash_attn (host f32) is only 0.19% — not the bottleneck**, contradicting the guess above. Root cause matches the pre-existing Llama finding (decode is device-bound on matmul launch overhead, ~26µs/tile) — gemma4 has 9 small matmul launches/layer × 35 layers ≈ 315 launches/token, more than Llama, so it's hit harder. Further gemma4 speedup would mean reducing matmul launch count / kernel work, not attention.
