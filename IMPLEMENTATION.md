...
- **Result:** correctness preserved throughout (golden `test_e2e_gemma` — France → Paris — passes after every phase); decode/prefill measured at ~1.6 forward-passes/sec post-P3 (13 passes — 6 prompt + 7 generated — in ~21.5s wall including model load), up from the ~1 tok/s pre-offload baseline. **flash_attn remains host f32** (unchanged, matches Llama/Qwen2's default and the pre-existing architectural block on a fused NPU attention kernel) and is likely now the dominant remaining cost — not re-measured in isolation this pass.
