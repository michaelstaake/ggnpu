...
- [~] **Phase G3 — PLE pathway.** *(implemented 2026-07-01; output now real words, not yet coherent)*
  - PLE built per token: token-identity (`per_layer_token_embd[tok]` × √ple_dim) + context-aware (RMSNorm(`per_layer_model_proj`(embed) / √hidden)), combined ×(1/√2); injected per layer via `post_norm(proj(gelu(inp_gate(h)) · pli_L))`.
  - **Forward-math fixes found via the reference graph + magnitude debugging:** RMSNorm uses **raw** weight (not `(1+w)` — gemma4 stores the full scale; weights are ~10–100, not ~0); `layer_output_scale` applied as a **delta scale** (`h = h_in + os·(h − h_in)`, os≈0.02–0.88 per layer) — moved output from multilingual garbage → real English tokens. Q4_0 matmul validated numerically correct (rel-L2 ≈ 1.9% = int8 activation-quant noise) via an in-context CPU-dequant check (`GGNPU_GEMMA_DEBUG=1`).
  - **Blocked on coherence:** output is real-but-repetitive (`"...France is" → " is is is"`); top logits saturate the softcap and favor the last token (copying). Remaining candidates (exact `out_scale`/PLE form, KV-sharing source, RoPE NeoX-vs-normal, attention scale) can't be isolated without **per-layer numerical ground truth** — the reference-oracle gap raised at G2. See `GGNPU_GEMMA_DEBUG` instrumentation in `main.cpp`.
