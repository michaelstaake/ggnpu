...
void decode_q3_k_flat(const uint8_t* gguf_data, size_t data_size,
                      std::vector<int8_t>& int8_output,
                      std::vector<float>& scales_output) {
    size_t num_blocks = data_size / Q3_K_BLOCK_BYTES;
    size_t n = num_blocks * QK_K;
    std::vector<float> f32(n);

    float max_abs = 0.0f;
    for (size_t i = 0; i < num_blocks; i++) {
        dequant_q3_k_block(gguf_data + i * Q3_K_BLOCK_BYTES, f32.data() + i * QK_K);
...
// Decode Q3_K weights to per-row INT8 + one scale per output row, matching the
// kq_path convention shared by the other k-quants.
void decode_q3_k_for_npu(const uint8_t* gguf_data, size_t data_size,
                         int64_t n_rows, int64_t n_cols,
                         std::vector<int8_t>& int8_output,
                         std::vector<float>& scales_output) {
    if (n_rows <= 0 || n_cols <= 0) {
        decode_q3_k_flat(gguf_data, data_size, int8_output, scales_output);
        return;
    }
