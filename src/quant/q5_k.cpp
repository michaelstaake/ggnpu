...
void decode_q5_k_flat(const uint8_t* gguf_data, size_t data_size,
                      std::vector<int8_t>& int8_output,
                      std::vector<float>& scales_output) {
    size_t num_blocks = data_size / Q5_K_BLOCK_BYTES;
    size_t n = num_blocks * QK_K;
    std::vector<float> f32(n);

    float max_abs = 0.0f;
    for (size_t i = 0; i < num_blocks; i++) {
        dequant_q5_k_block(gguf_data + i * Q5_K_BLOCK_BYTES, f32.data() + i * QK_K);
...
// Decode Q5_K weights for NPU with per-row symmetric int8 scales.
// GGUF 2D layout: n_cols (K) = dims[0], n_rows (N) = dims[1].
void decode_q5_k_for_npu(const uint8_t* gguf_data, size_t data_size,
                         int64_t n_rows, int64_t n_cols,
                         std::vector<int8_t>& int8_output,
                         std::vector<float>& scales_output) {
    if (n_rows <= 0 || n_cols <= 0) {
        decode_q5_k_flat(gguf_data, data_size, int8_output, scales_output);
        return;
    }
