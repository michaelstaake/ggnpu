// SPLIT-K decode matmul transform (hand-edited from matmul_small_m_aie2p.mlir).
// The small-M herd uses only 2 of the array's 8 columns (M_groups=2 x N_groups=4
// rows = 8 cores). This splits the K reduction across parallel threads to fill
// the free columns: each thread reduces K/split, and a combine op sums the
// per-thread partials. Goal: convert idle columns into real K-throughput at M=1.
...
module attributes {transform.with_named_sequence} {
  transform.named_sequence @__transform_main(%arg1: !transform.any_op {transform.readonly}) {
    ...
    //==========================================================================
        ...
        // SPLIT-K (before packing): partition the raw matmul K reduction (dim 2)
        // across 2 parallel threads. The per-thread partial matmul is then packed
        // + herded by the existing pipeline (pack feeds the partial directly, so
        // pack-fusion still works); the combine op sums the partials into C.
        %splitk_fill, %splitk_partial, %splitk_combine, %splitk_forall =
          transform.structured.tile_reduction_using_forall %matmul_to_pack
            by num_threads = [0, 0, 2]
            : (!transform.any_op) -> (!transform.any_op, !transform.any_op, !transform.any_op, !transform.any_op)
        transform.annotate %splitk_combine "splitk_combine" : !transform.any_op
        transform.annotate %splitk_forall "splitk_forall" : !transform.any_op

        %packed = transform.structured.pack %splitk_partial packed_sizes = [8, 8, 8]
    ...
    // Tile packed K dim by 8 (= 64 raw K elements). K_p is now half (split-K),
    // so this iterates half as many times per thread.
    //==========================================================================
    ...
    //==========================================================================
        ...
        %tiled_matmul_1, %inner_forall =
          transform.structured.tile_using_forall %matmul_1 tile_sizes [1, 8, 0] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
